...
module.exports = {
    getHotArticles: function (num, cb) {
        const NUM_PAGES = Math.floor(3 * num / 20);
        const fetchPromises = [];
        for (let i = 0 ; i < NUM_PAGES; i++) {
            fetchPromises.push(
                fetch('https://www.ptt.cc/bbs/Gossiping/search?page=' + (i + 1) + '&q=recommend%3A99', {
                    headers: {
                        'cookie': 'over18=1' 
                    }
                })
        ...
        Promise.all(fetchPromises).then((pagesData) => {
            ...
            const parsedData = allData.map((item) => {
                ...
                if ((!uniqueTitle[item.title]) && item.type && item.title) {
                    if (item.type.match(/公告/) || item.title.match(/女|寶|錢|取|疫|套|刮|車|性|覺|母|火|妹|奶|生|本|疫/)) {
                        return false;
                    } else {
            ...
            for (let i = 0; i < parsedData.length; i++) {
                ...
                let k = Math.floor(Math.random() * parsedData.length);
                if (j !== k) {
